...
def test_GaussRBF():
    ...
    rbf1.setTrustRegion(np.zeros(3), np.infty)
    rbf2.fit(x_vals_full, y_vals_full)
    rbf2.setTrustRegion(np.zeros(3), np.infty)
    ...
    rbf3.setTrustRegion(np.zeros(3), np.infty)
    ...
    rbf4.setTrustRegion(np.zeros(3), np.infty)
    ...
    rbf5.setTrustRegion(np.zeros(3), np.infty)
    ...
    rbf6.setTrustRegion(np.zeros(3), np.infty)
    ...
    rbf7.setTrustRegion(np.zeros(3), np.infty)
    # Test save and load
    ...
    rbf8.setTrustRegion(np.zeros(1), np.infty)
...
def test_LocalGaussRBF():
    ...
    with pytest.raises(ValueError):
        GaussRBF(2, np.zeros(3), np.ones(3), {'nugget': -1.0})
    with pytest.raises(ValueError):
        GaussRBF(2, np.zeros(3), np.ones(3), {'n_loc': []})
    with pytest.raises(ValueError):
        GaussRBF(2, np.zeros(3), np.ones(3), {'n_loc': 1})
    with pytest.raises(ValueError):
        GaussRBF(2, np.zeros(3), np.ones(3),
                      {'nugget': 0.1, 'des_tols': np.zeros(3)})
    with pytest.raises(ValueError):
    ...
    # Check that the RBFs compute the same grad, up to 8 digits of precision
    rbf1.setTrustRegion(0.5 * np.ones(3), np.ones(self.n) * 0.1)
    rbf2.setTrustRegion(0.5 * np.ones(3), np.ones(self.n) * 0.1)
    for i in range(x_vals_full.shape[0]):
        assert (np.linalg.norm(rbf1.gradient(x_vals_full[i]) -
                               rbf2.gradient(x_vals_full[i])) < 0.00000001)
        assert (np.linalg.norm(rbf1.stdDevGrad(x_vals_full[i]) -
    ...
    rbf3.fit(x_vals3, y_vals3)
    rbf3.setTrustRegion(x_vals3[-1], np.ones(3))
    ...
    rbf4 = GaussRBF(2, np.zeros(3), np.ones(3), {'nugget': 0.0001})
    rbf4.fit(x_vals1, y_vals1)
    rbf4.setTrustRegion(x_vals1[0], 0.1)
    ...
    rbf6.fit(x_vals1, y_vals1)
    rbf6.update(x_vals2, y_vals2)
    rbf6.setTrustRegion(x_vals1[0], 0.1)
    # Now fit datapoints in a plane
    x_vals3 = np.zeros((4, 3))
    x_vals3[1, 0] = 0.1
    x_vals3[2, 0] = 0.2
    ...
    rbf8.fit(x_vals4, y_vals4)
    rbf8.setTrustRegion(np.array([0.5]), 0.1)
    assert (np.linalg.norm(rbf8.evaluate(np.array([0.5])) - 0.5) < 1.0e-8)
